-- 1. [INNER JOIN] List every student's name together with the course name and marks for every course they are enrolled in.
-- 2. [INNER JOIN] List each course along with its instructor's name and the department that instructor belongs to.
-- 3. [LEFT JOIN] List every course, along with the instructor's name if one is assigned, or NULL if not.
-- 4. [LEFT JOIN] Find all students who are NOT enrolled in any course.
-- 5. [RIGHT JOIN] List every instructor and the courses they teach, including instructors who teach no course.
-- 6. [RIGHT JOIN] Find all courses that currently have zero enrolled students.
-- 7. [FULL OUTER JOIN] Produce one combined list showing every student-course relationship, including students with no course and courses with no student.
-- 8. [CROSS JOIN] Generate every possible (Student, Course) pairing for students belonging to the 'Computer Science' department's courses only.
-- 9. [SELF JOIN] List every mentor along with the number of mentees they are currently mentoring.
-- 10. [SELF JOIN] Find students who are BOTH a mentor to someone AND a mentee of someone else.
-- 11. [Multi-table JOIN] List each department along with the total number of students enrolled in courses run by that department (via Courses -> Instructors -> Departments -> Enrollments).
-- 12. [Aggregate + JOIN] Find the average marks per course, showing the course name, ordered from highest average to lowest.
